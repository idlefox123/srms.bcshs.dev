...
function getData() {
  ...
  $student = new Grading($_POST['ay'],$_POST['semester']);
  $student = $student->getEnrolleeID($_SESSION['userID']);
  ...
  $DB->loadResult("SELECT grade_id, ions.ion, subjects.subject_name, first_quarter, second_quarter, final FROM $grades
          ...
          WHERE $enrollment.enrollment_id = $student ORDER BY FIND_IN_SET(ion, 'Core,Applied,Specialized')");
  $data = array();
